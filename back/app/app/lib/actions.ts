...
const UpdateTournament = FormSchema.omit({ id: true, status: true, date: true });
...
export async function updateTournament(
  ...
  if (!validatedFields.success) {
    return {
      errors: validatedFields.error.flatten().fieldErrors,
      message: 'No se pudo actualizar el torneo, contacte con el administrador',
    };
  }
  ...
  try {
    await sql`
      ...
      name = ${name},
      image = NULLIF(${imagePosted}, image)::text,
      date = ${date}
      WHERE id = ${id}
    `;
  } catch (error) {
